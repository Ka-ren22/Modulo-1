...
function cacheFunction(cb) {
  ...
  // usá hasOwnProperty!
  var cache ={}; //Creo un objeto vacío para que guarde los valores del caché
    return function(x){  //A la función que retorne le voy a pasar un solo parámetro "x"
    if (!cache.hasOwnProperty(x)) {
      cache[x] = cb(x); //bracket notation: prop del objeto vacío
    } 
    return cache[x];
  }
}
...
*/
/*var = function textoAsteriscos (){
        return crearCadena;
      }
var  = function textoGuiones(){
  return crearCadena; 
};
var  = function textoUnderscore(){
  return crearCadena;
};
*/
...
let textoAsteriscos = crearCadena.bind(null, '*','*'); //el 1° argumento del bind es this, pero como no tengo su referencia lo completo con null
